...
create function public.reserve_inventory(inventory_id uuid)
...
begin
  ...
  where si.id = reserve_inventory.inventory_id
  for update;
  ...
  values (reserve_inventory.inventory_id, v_email, now(), now() + interval '24 hours', 'active')
...
-- refrescar schema cache de PostgREST (clave para que deje de decir "schema cache")
